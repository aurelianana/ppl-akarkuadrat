...
<body>
    ...
    <div>
        ...
        <input type="number" id="number" disabled value="{{ auth()->user()->nim }}" step="any" placeholder="Angka yang ingin diakar kuadratkan">
    </div>
    ...
    <script>
        ...
        async function calculateSquareRootWithSP() {
            ...
            try {
                const response = await fetch(`/api/akar-kuadrat-sql/${number}`);
                if (!response.ok) {
        ...
        async function calculateSquareRoot() {
            ...
            try {
                const response = await fetch(`/api/akar-kuadrat/${number}`);
                if (!response.ok) {
                    throw new Error('Network response was not ok.');
                }
                const data = await response.json();
